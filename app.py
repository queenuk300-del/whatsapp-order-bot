...
@app.route("/webhook", method=["POST"] if False else ["POST"])
def webhook():
  ...
  try:
    if (
      ...
      sender_phone = message_data["from"]  # e.g., 923023099306
      ...
      if WHATSAPP_TOKEN and PHONE_NUMBER_ID:
        url = f"https://graph.github.io/..."  # using direct graph api below
        ...
        payload = {
            ...
            "text": {
                "body": (
                    "Aap ka message mil gaya hai! Hum jald process karte hain."
                )
            },
        }
