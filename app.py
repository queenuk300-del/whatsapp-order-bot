...
6. **Final Alert Format:** JAB customer apna Naam aur Address dono de de, toh aapko apna aakhri jawab STRICTLY is format mein dena hai:
...
def webhook():
    ...
    try:
        if (
            ...
            # Hard system reset if things get stuck
            ...
            try:
                ...
                if "||ORDER_DONE||" in clean_text:
                    ...
                    # Send image with text if images are found
                    if images_to_send:
                        # Pheli tasweer ke sath AI ka sara text bhej dein
                        send_whatsapp_image(sender_phone, images_to_send[0], clean_text)
                        # Agar ek se zyada items ki tasweer ho, toh baqi bhi bhej dein
                        for img in images_to_send[1:]:
                            send_whatsapp_image(sender_phone, img, "")
                    else:
                        # Send normal text without images
